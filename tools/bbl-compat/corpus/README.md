...
| `nocte-cli/` | generated by `../make_corpus.ps1` with the NØCTE CLI | that what NØCTE Slicer *writes* is still native to Bambu Studio, subtypes and per-part overrides included |
...
## 3. `nocte-cli/` — the synthetic corpus written by NØCTE Slicer

Four projects produced end to end by the NØCTE CLI, so the harness tests the
*writer* as well as the reader. Unlike `bambu-studio/`, nothing here is authored
by hand: regenerate the whole directory with
...
powershell -File tools\bbl-compat\make_corpus.ps1 -SlicerDir <dir> -Oracle
```

Layout, one directory per case:

```
corpus/nocte-cli/NN_<name>/
    <name>.3mf             the corpus entry, written by the NØCTE writer
    <stem>.stl             the 12-triangle source meshes it was built from
    <name>.expected.json   manifest of what the harness must find
```

| directory | parts | per-part overrides |
| --- | --- | --- |
| `01_single_cube` | `normal_part` | – |
| `02_modifier_block` | `normal_part` + `modifier_part` | `sparse_infill_density`, `wall_loops` |
| `03_negative_part` | `normal_part` + `negative_part` | – |
| `04_support_enforcer_blocker` | `normal_part` + `support_enforcer` + `support_blocker` | – |

How they are built (three stages, because the CLI has no "add a modifier"
switch), the dead ends, and the round-trip results are documented in
`../NOTES.md` §7.

`<name>.expected.json` follows the same idea as `bambu-studio/expected.json` and
adds the fields that only apply to a generated file:

- `written_by` — `"nocte-cli"` when the file is the stage C re-export (the normal
  case), `"nocte-cli+patch"` if the NØCTE round trip lost something and the
  hand-patched stage B file had to be kept instead;
- `slicer_build` — the CI run id of the binary that wrote it;
- `findings` — empty when the round trip was clean;
- `nocte_tag` and `nocte_report` — both carry `"required_from_build": "block2"`,
  so a file produced by an older binary is not failed for lacking
  `<metadata name="NocteSlicer">` in `3D/3dmodel.model` or the archive entry
  `Metadata/nocte_report.json`. `present_in_this_file` records what the current
  file actually has.

The per-part `matrix` value is **not** an invariant: every writer in the Bambu
dialect, ours and Bambu Studio's alike, re-compounds it on each save
(`../NOTES.md` §7.4). `structdiff.py` reports it as a `layout` difference.
...
`make_corpus.ps1` already rewrites the manifest over the *whole* corpus at the
end of every run, so this snippet is only needed after touching `bambu-studio/`
or `fdmhub/` by hand. Run it from `tools/bbl-compat/`:
